...
public class GamemodeSurvival implements CommandExecutor
{
    ...
    public GamemodeSurvival(BobbieePlugin plugin)
    {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args)
    {
        ...
        if(sender instanceof Player)
        {
            ...
            if(sender.hasPermission("bobbieplugin.survival") || sender.hasPermission("bobbieplugin.gamemode"))
            {
                // checking to see if they specified a player
                if(args.length < 1)
                {
                    ((Player) sender).setGameMode(GameMode.SURVIVAL);
                    sender.sendMessage(ChatColor.GREEN + "Your gamemode has been changed to survival.");
                }
                ...
                else if(args.length > 1)
                {
                    ...
                    else
                    {
                        ...
                        sender.sendMessage(ChatColor.GREEN + "The player's gamemode has been changed to survival.");
                    }
                }
            }
