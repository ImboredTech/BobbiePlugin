...
public class GamemodeCreative implements CommandExecutor
{
    ...
    public GamemodeCreative(BobbieePlugin plugin)
    {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args)
    {
        ...
        if(sender instanceof Player)
        {
            ...
            if(sender.hasPermission("bobbieplugin.creative" ) || sender.hasPermission("bobbieplugin.gameode"))
            {
                // checking to see if they specified a player
                if(args.length < 1)
                {
                    ((Player) sender).setGameMode(GameMode.CREATIVE);
                    sender.sendMessage(ChatColor.GREEN + "Your gamemode has been changed to creative!");
                ...
                } else if(args.length > 1)
                {
                    ...
                    else
                    {
                        ...
                        sender.sendMessage(ChatColor.GREEN + "The player's gamemode has been changed to creative.");
                    }
                }
            }
